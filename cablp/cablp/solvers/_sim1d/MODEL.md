...
Unknown keys raise `ValueError`; `config_manifest()` exposes all 249 parameter
defaults and 47 flags with their defining groups (count current as of the
`cathode_jet_hot_carrier` build, 2026-08-21, which added the one parameter
the D3 deletion pass's 248 did not have). The live but formerly
