...
it, rather than reaching the solver's own construction-time refusal.
...
class Stance:
    ...
    declares -- the delta, not a full config. Use :func:`stance_config` for the
    resolved ``(params, flags)`` pair.
    """
...
def load_stance(name):
    ...
    unknown table, on a key its namespace does not own, and on a key named in
    ``[none_valued]`` that also carries a value.
    """
