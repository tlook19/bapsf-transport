...
| `cablp/vars/_nn_table.py` | **delete**, R0.4 retirement. Frozen, ungenerable (its generator is the untracked `scripts/generate_nn_table.py`), one consumer (`core/config.py:3` `lookup_nn0`), and production short-circuits it. **Not golden-inert in the source sense**: `core/config.py` must drop the import and the `resolve_nn0` branch that calls it, which is a solver-file edit inside R2. See Q3. **ANNOTATION 2026-08-27 (execution):** "production short-circuits it" is WRONG, and so is "one consumer". The GOLDEN reached the table — `build_baseline_config()` yields `nn0 = None` with `neutral_initial_profile` cleared, so `resolve_nn0` fell through and the gate's uniform fill was `lookup_nn0(9010.0) = 2.725059978765871e12`. `scripts/sp3_build_nn0.py:394` is a second consumer (**cite corrected 2026-08-27**: this annotation first read `:385`, the line at the retirement's own base; the retirement's own edit to that call site moved it, so `:394` is where the surviving `resolve_nn0(params)` call reads at and after `02333ce`). Retirement executed under Tom's option (a) ruling: the value was pinned as an explicit literal in `baseline_sim1d.py` FIRST (bit-identical initial state; digest identity rotated, trajectory unmoved), then the table was deleted. |
| `cablp/vars/nn_table.csv` | **delete**, the data half of the same retirement. |
| `cablp/solvers/_sim1d/results/compat.py` | **delete**, R0.4 retirement (sim3 aliases for a solver removed at D2). Three consumers: `results/io.py:16,380`, `solver.py:205,10153`, and — **the one that bites** — `scripts/smoke_sim1d.py:8074-8077`, which asserts on `sim3_compat_units` and `sim3_compat_notes`. See §6.4 and Q3. **ANNOTATION 2026-08-27 (execution):** there were FOUR consumers, not three — `scripts/sweep_sim1d_stability.py:163,166` reads `result.cathode.I_tot` / `result.cathode_twin.I_tot` and is missing from this row and from Q3. The smoke count is also understated: not four assertions at `8074-8077` but five blocks across three cases, and those line numbers are pre-flatten. Both counts were cut at a stale revision; the retirement re-derived the set at its own base and migrated the sweep script to `cathode_diagnostics`. |
...
**ANNOTATION 2026-08-27 (execution).** Both halves of this section were
understated, and the shape of the error is the same in each: the census behind
it was cut at a revision older than the one it was read against.

- **compat.** Not four assertions at those two lines but five blocks across
  three smoke cases, plus one consumer OUTSIDE the smoke that no row here
  names — `sweep_sim1d_stability.py`, reading the `cathode` / `cathode_twin`
  namespaces. The line numbers are pre-flatten. Executed by re-deriving the
  consumer set at the retirement's own base.
- **`_nn_table`.** "The golden fixture is unaffected" holds for the *fixture*
  and NOT for the *gate*: the golden's config reached `lookup_nn0`, so
  dropping the call site alone made the golden unconstructible. The
  retirement therefore ran in two commits under Tom's ruling — pin the
  table's value as an explicit literal in the golden builder, then delete the
  table — with the pin proved to leave the constructed initial state
  bit-identical and the 4k trajectory digest unmoved.

The general lesson for the remaining R2 seams: a retirement's consumer census
is only as good as the revision it was taken at, and a "production
short-circuits it" claim is a claim about EVERY config that ships, gate
configs included.
