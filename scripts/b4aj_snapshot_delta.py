...
The B4 twin of ``b5cj_snapshot_delta.py``, and the same statement: adding
configuration keys moves every digest in
...
snapshot case on the current tree, REMOVES the four keys this branch adds, and
checks that the digest returns to the value a named base commit recorded. If
every case comes back, the rotation carries no value change.
...
``anode_jet_R_N`` / ``anode_jet_R_E`` -- so "no value moved" covers the new
keys as well as the old ones.

Usage (from the checkout root, PYTHONPATH set to it)::

    python scripts/b4aj_snapshot_delta.py [--base <rev>]
"""
