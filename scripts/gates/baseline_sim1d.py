...
recapture casually: a recapture is reviewed, authorized, and recorded, and that
protocol and its record are held outside this repository. What a recapture
writes HERE is the committed sidecar ``baselines/production_discharge.json``,
rewritten beside the fixture at every capture, and that sidecar is the in-repo
authority for what this golden was captured at -- its configuration, its
resolved params and flags, and the run the fixture holds.
...
#
# The capture's own measured cost -- step count, dynamic end time and save
# count -- is rewritten at every recapture into the committed sidecar
# baselines/production_discharge.json (its "summary" block). It is deliberately
# NOT restated here: a runtime figure in a standing comment has no way of
# announcing that it has drifted, while a sidecar regenerated by the capture
# cannot go stale.
#
# max_steps is a TRIPWIRE, not a run length: roughly twice the step count the
# capture actually measures (the sidecar carries that count), paired with
